 ...
 */
public class DatashareMetadataChecker {
	...
	// 	if (collection == null) {
	// 		throw new IllegalStateException("No collection found. Does user " + context.getCurrentUser().getEmail()
	...
	// 		List<MetadataValue> medataValues = itemService.getMetadataByMetadataString(item, field);
	// 		if (medataValues.size() == 0) {
	// 			if (field.equals(DatashareMetadataUtils.DEPOSITOR_STR)) {
	// 				// always ignore depositor
	// 			} else if (field.equals(DatashareMetadataUtils.PUBLISHER_STR)
	// 					&& itemService.getMetadataByMetadataString(item, DatashareMetadataUtils.CREATOR_STR).size() > 0) {
	...
	// 			}
	// 		} else {
	// 			if (medataValues.get(0).getValue().trim().length() == 0) {
	...
	// 		List<MetadataValue> medataValues = itemService.getMetadataByMetadataString(item, field);
	// 		if (medataValues.size() > 1) {
	// 			// we have a non repeatable field with more than one value
	// 			throw new IllegalStateException(
	// 					field + " is a non repeatable metadata field and cannot have more than one value");
	// 		}
	// 	}
	// }
	...
	//  * @throws SQLException
	//  */
	// public boolean isAllowedMetadataField(Context context, Item item, String element, String qualifier)
	// 		throws IllegalStateException, SQLException {
	// 	return isAllowedMetadataField(context, item, DC_SCHEMA, element, qualifier);
	// }
	...
	//  * @throws SQLException
	//  */
	// public boolean isAllowedMetadataField(Context context, Item item, String schema, String element, String qualifier)
	// 		throws IllegalStateException, SQLException {
	...
	// 		String mdField = DatashareDspaceUtils.getMdString(schema, element, qualifier);
	...
	// 	final List<String> exceptions = new ArrayList<String>(Arrays.asList("dc.identifier.govdoc",
	// 			"dc.identifier.isbn", "dc.identifier.ismn", "dc.identifier.issn", "dc.subject.ddc", "dc.language.iso"));
	...
	// 	required.add(DatashareDspaceUtils.getMdString("rights"));
	// 	allowed.add(DatashareDspaceUtils.getMdString("rights"));
	// 	allowed.add(DatashareDspaceUtils.getMdString("coverage", "temporal"));
	// 	allowed.add(DatashareDspaceUtils.getMdString("ds", "withdrawn", "showtombstone"));
	...
	// 				String field = DatashareDspaceUtils.getMdString(input.getSchema(), input.getElement(), input.getQualifier());
	// 				if (input.isRequired()) {
	// 					required.add(field);
	// 				}
