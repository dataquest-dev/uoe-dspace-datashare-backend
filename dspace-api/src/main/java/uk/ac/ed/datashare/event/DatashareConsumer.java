 ...
 *       {@code MODIFY_METADATA} touching the title) drops the stale zip and regenerates it from the
 *       current files.</li>
...
public class DatashareConsumer implements Consumer {
    ...
    /**
     * The metadata field, as it appears in a {@link Event#MODIFY_METADATA} event detail (see
     * {@link org.dspace.content.MetadataField#toString()}, which joins the parts with '_'), that
     * backs a bitstream's name. The dataset zip names each entry after its bitstream's
     * {@code dc.title}, so a change to this field - a file rename - makes the existing zip stale.
     */
    private static final String BITSTREAM_NAME_FIELD = "dc_title";
    ...
    public void consume(Context ctx, Event event) throws Exception {
        ...
        try {
            ...
            // A file packaged into the zip was renamed (its dc.title - and therefore the entry name
            // inside the zip - changed): the existing zip is stale, so drop it and regenerate it from
            // the current files (deleted here, recreated by the reconcile step below).
            Item renamed = resolveItemWhoseZipFileRenamed(ctx, event);
            if (renamed != null && renamed.isArchived()) {
                itemsToDelete.add(renamed.getID());
    ...
    /**
     * Work out, for a bitstream metadata change, the item whose dataset zip became stale because one
     ...
     * @param ctx   DSpace context
     * @param event the event being consumed
     * @return the owning item whose zip is now stale, or {@code null}
     * @throws Exception if the subject of the event cannot be resolved
     */
    private Item resolveItemWhoseZipFileRenamed(Context ctx, Event event) throws Exception {
        ...
        for (Bundle bundle : bitstream.getBundles()) {
            if (isZipBundle(bundle.getName())) {
                DSpaceObject parent = bitstreamService.getParentObject(ctx, bitstream);
                if (parent instanceof Item) {
                    return (Item) parent;
                }
            }
